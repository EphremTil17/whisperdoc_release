...
def _parse_optional_int(value: str | None) -> int | None:
    """Parse integer-like env values, treating blank as unset/default."""
    if value is None:
        return None
...
class ConfigService:
    ...
    def _load_env(self):
        ...
        self.AUDIO_DEVICE_ID = _parse_optional_int(os.getenv("AUDIO_DEVICE_ID"))
    ...
    def _parse_args(self):
        ...
        parser.add_argument(
            "--health", action="store_true", help="Run pre-flight server health check"
        )
        parser.add_argument(
            "--version", action="store_true", help="Print client version and exit"
        )
        parser.add_argument(
            "--clear-key", action="store_true", help="Clear stored API key and exit"
        )
        parser.add_argument(
            "--incognito",
            action="store_true",
            help="Enable Ghost Mode (No server logs, no local history)",
        )
