# WhisperDoc v2.24.6 (Public Release)
...
A high-performance, **multi-layered secure**, and production-ready speech-to-text system featuring a **pluggable multi-engine ASR architecture** (OpenAI Whisper, NVIDIA Parakeet) with GPU acceleration within a **hardened, read-only enclosure**, paired with modern, zero-trust client applications for seamless, identity-verified dictation. The Windows Flutter client can now operate either through the WhisperDoc backend or through a direct **Groq Cloud** transcription path when backend connectivity is unavailable or intentionally bypassed.

https://github.com/user-attachments/assets/a6034fd9-7f4b-4279-9e5b-acfc6238f626

## Purpose

WhisperDoc Client provides a lightweight, always-ready interface for voice dictation. It captures audio from your microphone and can either stream it to a self-hosted/public WhisperDoc backend server or transcribe it directly through Groq Cloud, depending on the active mode. The transcribed text can be automatically copied to your clipboard and pasted into any application.
...
Pre-built Windows installers are distributed with the release package. Download the latest `.exe` from the current Windows release artifact and run the installer.

### Prerequisites

- Windows 10/11 (x64)
- Either:
  - A running WhisperDoc backend server — you can self-host with Docker and an NVIDIA GPU, or connect to the public demo server at **`https://whisper.ephremst.com`**
  - A Groq Cloud API key if you want to use the client’s direct cloud transcription mode without the WhisperDoc backend

## Security & Protocol Infrastructure

Both clients share a hardened security and transport foundation for backend-connected operation, and the Flutter client applies the same security posture to its direct cloud mode where the backend transport path is not in use:
...
- **Handshake Cage Protocol**: A strict WebSocket state machine that buffers audio locally and only flushes to the socket _after_ the identity-verified handshake is acknowledged by the backend. The server is hardened to immediately drop and blacklist IPs that attempt to stream audio before successful identity verification. No data leaves the client until the connection is fully authenticated.
...
- **Structured Error Protocol**: WebSocket error payloads include both a numeric `code` and a structured `error_code` string (e.g., `AUTH_FAILED`, `VERSION_OUTDATED`, `IP_BANNED`) for precise client-side error routing. Both clients route backend failures with user-specific guidance and actionable messages.
...
- **Client-Side Sanitization**: Direct cloud transcription responses are sanitized on the client before they reach the UI, clipboard, or automation path, aligning the Groq mode text boundary with the backend sanitizer model.
...
### Flutter Client (Primary) — Windows v2.24.6
...
- **Dual-Path Transcription**: Supports both the WhisperDoc backend and a direct Groq Cloud mode with mode-aware settings, status indicators, provider-specific profile surfaces, local WAV packaging, and built-in client-side rate limiting.
...
- **Data-at-Rest Encryption**: Transcription history is stored in a local **Drift/SQLite** database with encrypted fields protected by installation-bound key material and the Windows credential boundary.
...
- **Advanced Security & Provider Indicators**: Visual feedback for transport security, blocked/banned states, backend readiness, and Groq Cloud mode so the active provider state is always explicit.
...
### Python Terminal Client — Windows Only v2.24.6
...
- **Transcription Mode**: Switch between WhisperDoc backend mode and Groq Cloud mode
- **Server URI**: WebSocket endpoint for backend mode (e.g., `wss://whisper.ephremst.com/ws` or `ws://localhost:9989/ws`)
- **Secure Key**: API Key or JWT for backend mode (stored in Windows Credential Manager)
- **Groq API Key / Language / Prompt**: Direct-cloud settings used only in Groq mode
...
- [release_notes/CHANGELOG.md](release_notes/CHANGELOG.md) — Technical changelog across all versions
